...
fragment float4 surfaceEffects(VertexOut in [[stage_in]],
    ...
    float confidence = 0;
    if (reflectivity > .015) {
        // Jitter the march start per pixel so the exponential step boundaries
        // become sub-texel noise instead of visible concentric bands.
        ...
        for (uint step=0; step<48; ++step) {
            ...
            if (delta > 0 && previousDelta <= 0) {
                ...
                if (valid > .01) {
                    ...
                    float2 blur = s.texel*2.5;
                    reflection=(color.sample(linearSampler,hitUV+blur).rgb+color.sample(linearSampler,hitUV-blur).rgb+
                        color.sample(linearSampler,hitUV+float2(blur.x,-blur.y)).rgb+color.sample(linearSampler,hitUV+float2(-blur.x,blur.y)).rgb)*.25;
                    break;
                }
            }
            previousDelta=delta;
